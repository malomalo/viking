...
import {forEach as _each, remove as _remove, uniqueId as _uniqueId} from 'lodash';
...
export default class EventBus {
  ...
  addEventListener(name, callback, options?) {
    ...
    const {once = false, context = undefined, listener = undefined} = options || {};
  ...
  addEventListenerFor(obj, name, callback?, options?) {
    ...
    if (!obj.listenerId) { obj.listenerId = _uniqueId('b'); }
  ...
  removeEventListener(name?, callback?, options?) {
    ...
    const filter = (handlers, key) => {
      if (!handlers) { return; }

      _remove(handlers, (handler: IEventListener) => {
        if (
    ...
    if (name) {
      ...
      _each(this.eventListeners, filter);
    }
    ...
    dispatchEvent(name: string | string[], ...args) {
        if (Array.isArray(name)) {
          name.forEach((v, i) => this.dispatchEvent(v));
          return this;
        }
        ...
        _remove(this.eventListeners[name], (callback, index) => {
          if (callback) {
        ...
        _remove(this.eventListeners['*'], (callback, index) => {
