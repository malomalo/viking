...
import {clone as _clone, isEqual as _isEqual, each as _each, defaults as _defaults, result as _result, uniqueId as _uniqueId} from 'lodash';
...
export class Model extends EventBus {
    ...
    static abstract: boolean = true;
    static primaryKey: string = 'id';
    static schema: IModelSchema;
    static cidPrefix: string = 'm';

    // inheritanceAttribute is the attirbutes used for STI
    static inheritanceAttribute: string | boolean = 'type';
    ...
    static baseModel(): any {
        if (
             ((this as any).__proto__.hasOwnProperty('abstract') && (this as any).__proto__.abstract)
             ||
             (this as any).__proto__.getInheritanceAttribute() === false
            ) {
            return this;
        } else {
            return (this as any).__proto__.baseModel();
            // this.baseModel.descendants.push(this);
        }
    }
    ...
    id: any;
    cid: string = _uniqueId((this.constructor as any).cidPrefix);
    inheritanceAttribute: string | boolean = (this.constructor as any).getInheritanceAttribute();
    ...
    constructor(attributes?: IModelAttributes, options?: IModelOptions) {
        ...
        if (this.constructor !== Model) {
            this.modelName = (this.constructor as any).modelName();
            this.baseModel = (this.constructor as any).baseModel();
        } else {
...
//         }
        ...
        attributes = Object.assign({}, this.defaults(), attributes);
        if (attributes) {
    ...
    urlRoot: string | (() => string) = () => {
        return _result(this.constructor, 'urlRoot');
    }


    defaults() {
        let dflts = {};
        _each((this.constructor as any).schema, (options, key) => {
            if (options.default) {
    ...
    setAttributes(attributes: object) {
        ...
        _each(attributes, (value, key) => {
            let coervedValue = undefined;
            let schema = (this.constructor as any).schema;
            if (schema && schema[key] && schema[key].type) {
                if (schema[key].array) {
                    coervedValue = value.map((v) => Type.registry[schema[key].type].load(v));
                } else if (Type.registry[schema[key].type]) {
                    coervedValue = Type.registry[schema[key].type].load(value);
                } else {
            ...
            if (!_isEqual(this.attributes[key], coervedValue)) {
                changes.push(key);
            }
            this.attributes[key] = coervedValue;
        });

        if ((this.constructor as any).primaryKey in attributes) {
            this.id = this.readAttribute((this.constructor as any).primaryKey);
        }
    ...
    url(): string {
        let base = _result(this, 'urlRoot') as string;
