...
describe('Viking.Record::associations', () => {
    describe('hasMany(Parent)', () => {
        ...
        describe('.load()', () => {
            it("load association", function(done) {
                ...
                child.parents.toArray().then((models) => {
                    ...
                    assert.deepEqual(child, models[0].child)
                }).then(done, done);

                this.withRequest('GET', '/parents', { params: {where: {child_id: 24}, order: {id: 'desc'}} }, (xhr) => {
            ...
            it("sends one network request and return the same result to all callers of load while request is in flight", function(done) {
                ...
                let a = child.parents.load();
                let b = child.parents.load();
        ...
        it('coalesces loads when a inflight load is happening', function (done) {
            ...
            let p1 = child.parents.load().then(() => { callback_order.push(1); });
            let p2 = child.parents.load().then(() => { callback_order.push(2); });
        ...
        it("reload association", function (done) {
            let child = new Child({id: 24});
            child.parents.toArray().then(parents => {
                child.association('parents').reload().then(secondLoadParents => {
                ...
                assert.ok(this.findRequest('GET', '/parents', { params: {where: {child_id: 24}, order: {id: 'desc'}} }));
        ...
        it("add to association", async function () {
            ...
            child.parents.instantiate([])

            await child.parents.add(parent)
            await child.parents.add(parent2)
            assert.deepEqual([parent.cid, parent2.cid], child.parents.target.map(x => x.cid))
        })
        ...
        it("forEach iterates over the association", function(done) {
            ...
            child.parents.forEach((p) => { loaded_parents.push(p) }).then(() => {
        ...
        it("map maps over the association", function(done) {
            ...
            child.parents.map((p) => p.readAttribute('id')).then((ids) => {
        ...
        describe('assigning the association', () => {
            it('to a model with an id', async function() {
                let child = new Child({id: 13});
                let parents = [new Parent(), new Parent()];
                child.parents = parents;

                parents.forEach((m) => { assert.equal(m.readAttribute('child_id'), 13); });
                assert.ok(child._associations.parents.loaded);
                assert.deepStrictEqual(child._associations.parents.target, parents);
                assert.deepStrictEqual(await child.parents.toArray(), parents);
                assert.equal(this.requests.length, 0);
            });

            // it('to a child without an id', function(done) {
                //     let child = new Child();
            //     let parent = new Parent();
            //     child.parent = parent;

            //     assert.equal(child.readAttribute('parent_id'), null);
            //     assert.ok(child._associations.parent.loaded);
            //     assert.strictEqual(child._associations.parent.target, parent);
            //     child.parent.then((child) => {
            ...
            //     assert.equal(child.readAttribute('parent_id'), null);
            //     assert.ok(child._associations.parent.loaded);
            //     assert.strictEqual(child._associations.parent.target, null);
            //     child.parent.then((model) => {
        ...
        describe('addBang', () => {
            it('sends request', function () {
                let child = Child.instantiate({id: 24})
                let parent = Parent.instantiate({id: 11})
                child.association('parents').addBang(parent)

                assert.equal(this.requests[0].url, 'http://example.com/children/24/parents/11')
            ...
            it('updates target when loaded and record persisted', function (done) {
                ...
                child.parents.load().then(() => {
                    parent1.then((p) => {
                        child.parents.addBang(p).then(() => {
                            assert.deepStrictEqual(child.parents.target, [p])
                        }).then(done, done)

                        this.withRequest('POST', '/children/24/parents/11', {}, (xhr) => {
            ...
            it('doesnt update target when not loaded', function (done) {
                ...
                child.parents.addBang(parent1).then(() => {
                    assert.deepStrictEqual(child.parents.target, [])
                    assert.ok(!child.parents.loaded)
                }).then(done, done)
            ...
            it('creates target', function (done) {
                ...
                child.parents.load().then(() => {
                    child.parents.addBang(parent).then(() => {
                        assert.equal(parent.readAttribute('child_id'), 24)
                        assert.equal(parent.readAttribute('name'), 'Tim Smith Modified')
                        assert.equal(child.parents.target.length, 1)
                    }).then(done, done)

                    this.withRequest('POST', '/children/24/parents', {
                        name: 'Tim Smith'
                    }, (xhr) => {
                        xhr.respond(201, {}, '{"id": 1, "name": "Tim Smith Modified", "child_id": 24}');
                    });
                })
            ...
            it('raises error when parent model is not persisted', function () {
                ...
                assert.throws( () => child.parents.addBang(parent), Errors.RecordNotSaved );
            });
        })
        ...
        describe('removeBang', () => {
            it('sends request', function () {
                ...
                child.parent = [parent]
                child.parents.removeBang(parent)
            ...
            it('updates target', function (done) {
                ...
                child.parents.removeBang(parent).then(() => {
                    assert.equal(child.parents.target.length, 0)
                }).then(done, done)

                this.withRequest('DELETE', '/children/24/parents/11', {}, (xhr) => {
                    xhr.respond(201, {}, null);
                });
            })
        ...
        describe('remove', () => {
            it('same record', async () => {
                let parent = new Parent({id: 11})
                let child = new Child({id: 24, parents: [parent]})

                await child.parents.remove(parent);
                assert.deepEqual(child.parents.target, []);
            })
            ...
            it('clone of record', async () => {
                ...
                await child.parents.remove(parentClone);
                assert.deepEqual(child.parents.target, []);
            })
        })
    ...
    describe('hasMany(Parent, {inverse_of: "child"})', () => {
        class Parent extends VikingRecord {
            static associations = [belongsTo('offspring', Child)]
        }
        function Child () {
            var NewTarget = Object.getPrototypeOf(this).constructor;
            return Reflect.construct(VikingRecord, arguments, NewTarget);
        ...
        extendClass('Child', VikingRecord, Child, {
            associations: [hasMany(Parent, {inverseOf: 'offspring'})]
        })
        
        it("load association", function(done) {
            let child = new Child({id: 24});

            child.parents.load().then((parents) => {
                assert.ok(parents[0].association('offspring').loaded)
                assert.deepEqual(child, parents[0].offspring)
                assert.equal(child.cid, parents[0].offspring.cid)
            }).then(done, done);
            this.withRequest('GET', '/parents', {params: {where: {child_id: 24}, order: {id: 'desc'}}}, (xhr) => {
                xhr.respond(200, {}, '[{"id": 11, "name": "Viking", "child_id": 24}]');
            });
        });
    })
