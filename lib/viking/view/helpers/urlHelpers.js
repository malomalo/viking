...
import { contentTag } from 'viking/view/helpers/tagHelpers';
import { each } from 'viking/support';
import { escape as escapeHTML } from 'viking/support/escape';

let mailToOptions = ['cc', 'bcc', 'body', 'subject'];
...
export function linkTo(content, modelOrUrl, options) {
    ...
    return contentTag('a', content, Object.assign({ href: urlFor(modelOrUrl) }, options), false);
};
...
export function mailTo(email, name, options) {
    var tmp;
    ...
    let extras = [];
    mailToOptions.forEach((k) => {
        if (options[k]) {
            extras.push(k + '=' + encodeURIComponent(options[k]));
        }
    });
    extras = extras.join('&');
    ...
    each(options, (key, value) => {
        if (!mailToOptions.includes(key)) {
            tagOptions[key] = value;
        }
    });

    tagOptions.href = "mailto:" + email + extras;

    return contentTag('a', name, tagOptions, false);
};
...
export function urlFor(modelOrUrl, options) {
    ...
    let route;
    const klass = modelOrUrl.baseClass.modelName.model();
    if (modelOrUrl instanceof klass) {
        if (modelOrUrl.isNew()) {
            route = constantize(klass.baseClass.modelName.plural + 'Path');
            route = route();
        } else {
            route = constantize(klass.baseClass.modelName.singular + 'Path');
            route = route(modelOrUrl);
        }
    } else {
        route = constantize(modelOrUrl.baseClass.modelName.plural + 'Path');
        route = route();
    }
