...
export default class Record extends Model {
    ...
    // Instance state used by Record is set up in `initialize` (below) rather than
    // as class fields. Field initializers run only after `super()` returns — i.e.
    // after Model's constructor has already invoked `initialize` — so declaring
    // these as fields would reset them and wipe the attached associations.
    ...
    /**
     * Sets up Record-specific state (persistence flags, associations), then
     * delegates to {@link Model#initialize} to apply defaults and attributes.
     * Associations are attached first so attribute application can resolve them.
     * @param {Object} attributes - The attributes and associations to apply
    ...
    /**
     * Applies the initial attributes and associations. Overrides
     * {@link Model#applyAttributes}.
     * @param {Object} attributes - The attributes and associations to apply
     * @param {Object} options - Construction options
     */
    applyAttributes(attributes, options) {
