...
//         }
        ...
        each(attributes, (key, value) => {
            if (this._associations.hasOwnProperty(key)) {
                ...
                if (value instanceof Record) {
                    ...
                    association.setAttributes(value);
                }
            }
        })
    ...
    saveAttributes(options) {
        ...
        if (options.include) {
            each(Object.keys(options.include), association_name => {
                const assocation = this.association(association_name)
                const includes = options.include[association_name]
                const association_attributes = assocation.saveAttributes({
                    include: includes
                })
                
                if (Array.isArray(association_attributes) && association_attributes.length == 0) {
                    return
                } else if (Object.keys(association_attributes).length == 0) {
                    return
                }
                attributes[`${assocation.reflection.name}_attributes`] = association_attributes
            })
        }
    ...
    async save(options = {}) {
        ...
        options.body = deepAssign({ [this.modelName.singular]: attributes }, options.body)
        options.success = (response) => {
            this.setAttributesAndAssociations(response);
            let savedChanges = {};
