...
//         }
    ...
    setAttributes(attributes, coerced=false, persisted=false) {
        let changes = {};
        ...
        each(attributes, (key, value) => {
            if (!(key in this.attributes) || !isEqual(this.attributes[key], value)) {
                ...
                if (this._changes[key]) {
                    if (persisted || isEqual(this._changes[key][0], value)) {
    ...
    optionsForSync(event, options={}, attributes=null) {
        ...
        options.success = (response) => {
            ...
            if (event == 'updateAttributes') {
                savedChanges = this.setAttributes(pick(response, ...Object.keys(attributes)), false, true);
            } else {
