...
//  |saved|record has successfully saved to api||
//  |invalid|record has failed to save because it is invalid|errors|
...
//         }
    ...
    async save(options = {}) {
        ...
        options.body = deepAssign({
            [this.modelName.singular]: this.asJSON()
        }, options.body)
        ...
        options.error = (request, error_callback) => {
            if (request.status === 400 && request.getResponseHeader('Content-Type').startsWith('application/json')) {
                ...
                this.dispatchEvent('invalid', this.errors);
                return false;
            }
        }
    ...
    async destroy() {
        ...
        await this.constructor.connection.delete(this.url());
