...
//         }
    ...
    optionsForSync(event, options={}) {
        ...
        let changes
        if (event == 'save') {
            changes = options.changes || this.changes();
            if (wasNew) {
        ...
        if (event == 'save') {
            let attributes = {}
            if (options.changes) {
                each(changes, (attribute, [was, now]) => attributes[attribute] = now)
                attributes = this.dumpAttributes(attributes)
            } else {
            ...
            if (options.body && isPlainObject(options.body)) {
                options.body = deepAssign({ [this.paramRoot()]: attributes }, options.body);
            } else if (!options.body) {
                options.body = { [this.paramRoot()]: attributes};
            }
        }
        ...
        options.success = (response) => {
            ...
            if (changes) {
                ...
                each(response, (attribute, value) => {
                    const unsavedChange = this._changes[attribute]
                    if (!(attribute in previousChanges) && unsavedChange) {
                        unsavedChange.splice(0, 1, value)
                        unsavedChanges[attribute] = unsavedChange
    ...
    update(attributes={}, options={}) {
        ...
        each(attributes, (attribute, value) => {
            if (this.hasChanged(attribute) || !isEqual(this.attributes[attribute], value)) {
                updateChanges[attribute] = [this._changes[attribute]?.at(0) || this[attribute], value]
            }
        })

        if (Object.keys(updateChanges).length == 0) { return }
        
        options.changes = updateChanges
