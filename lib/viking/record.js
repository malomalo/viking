    ...
    optionsForSync(event, options={}, attributes=null) {
        ...
        if (event == 'save') {
            if (options.body && isPlainObject(options.body)) {
                options.body = deepAssign(this.constructor.connection.buildRequestBody(this), options.body);
            } else if (!options.body) {
                options.body = this.constructor.connection.buildRequestBody(this);
            }
        } else if (event == 'updateAttributes') {
            const connection = this.constructor.connection;
            options.body = connection.buildRequestBody(this, connection.dumpAttributes(this, attributes));
        }
        ...
        options.invalid = (request, error_callback) => {
            const errors = this.constructor.connection.parseErrors(request.responseText, request.getResponseHeader('Content-Type'));
