...
/**
 * A model class for handling data persistence and relationships. Viking.Record provides ActiveRecord-like
...
export default class Record extends EventBus {
     ...
     * @returns {Object} The association reflection
     */
     ...
     * @static
     * @memberof Viking.Record
     * @returns {Array<Object>} Array of association reflections
     */
     ...
     * @static
     * @memberof Viking.Record
     * @returns {Name} The model name object containing naming information
     */
    static modelName() {
     ...
     * @static
     * @memberof Viking.Record
     * @returns {typeof Record} The base class of this model
     */
    static baseClass() {
        if (
    ...
    /**
     * Generates the URL root based off of the class name
     * @static
     * @memberof Viking.Record
     * @returns {string} The URL root for API endpoints relating to this model type
     */
    static urlRoot() {
        if (this.path) {
            return this.path;
...
//         }
     ...
     * @returns {Record} This record instance for chaining
     * @fires changed - When attributes are changed, see {@link Viking.Record#event:changed}
     * @fires changed:{attribute} - When a specific attribute is changed, see {@link Viking.Record#event:changed:{attribute}}
     */
    setAttributes(attributes, coerced=false) {
        ...
        each(attributes, (key, value) => {
            if (!(key in this.attributes) || !isEqual(this.attributes[key], value)) {
                 ...
                 * @event Viking.Record#changed:{attribute}
                 * @param {Viking.Record} record - The record that was changed
     ...
     * @throws {Errors.RecordNotSaved} If the save operation fails
     * @async
     * @fires beforeCreate - Before a new record is created, see {@link Viking.Record#event:beforeCreate}
     * @fires beforeSave - Before a record is saved, see {@link Viking.Record#event:beforeSave}
     * @fires beforeSync - Before a record is synced with the server, see {@link Viking.Record#event:beforeSync}
     * @fires afterCreate - After a record is created, see {@link Viking.Record#event:afterCreate}
     * @fires afterSave - After a record is saved, see {@link Viking.Record#event:afterSave}
     * @fires afterSync - After a record is synced with the server, see {@link Viking.Record#event:afterSync}
     * @fires afterSync:{attribute} - After a specific attribute is synced, see {@link Viking.Record#event:afterSync:{attribute}}
     * @fires invalid - When validation fails, see {@link Viking.Record#event:invalid}
     * @fires error - When a server error occurs, see {@link Viking.Record#event:error}
     */
    async save$(options = {}) {
    ...
    optionsForSync(event, options={}) {
        ...
        if (event == 'save') {
            ...
            if (wasNew) {
                 ...
                 * @event Viking.Record#beforeCreate
             ...
             * @event Viking.Record#beforeSave
             * @param {Object} changes - The attributes that will be saved
             * @param {Object} options - Options for the save operation
             */
         ...
         * @event Viking.Record#beforeSync
        ...
        options.success = (response) => {
            ...
            if (event == 'save') {
                if(wasNew) {
                     ...
                     * @event Viking.Record#afterCreate
            ...
            each(savedChanges, (key, value) => {
                 ...
                 * @event Viking.Record#afterSync:{attribute}
                 * @param {*} oldValue - The previous value of the attribute
                 * @param {*} newValue - The new value of the attribute
                 */
     ...
     * @throws {Errors.ConnectionNotEstablished} If no connection is established
     * @async
     * @fires beforeDestroy - Before the record is destroyed, see {@link Viking.Record#event:beforeDestroy}
     * @fires afterDestroy - After the record is destroyed, see {@link Viking.Record#event:afterDestroy}
     */
    async destroy(options = {}) {
         ...
         * @event Viking.Record#beforeDestroy
         * @param {Viking.Record} record - The record being destroyed
        ...
        await this.constructor.connection.delete(this.url()).then(() => {
             ...
             * @event Viking.Record#afterDestroy
             * @param {Viking.Record} record - The record that was destroyed
             * @param {Object} options - Options for the destroy operation
             */
