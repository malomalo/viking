...
export default class Record extends Model {
    ...
    static instantiate(attributes) {
        let record = new this();
        record.setAttributes(attributes, {dirty: false})
        record.persist()
        return record;
    }
     ...
     * keys. (`collections` is provided by {@link Model}.)
     */
    declareProperties() {
    ...
    /**
     * Overrides {@link Model#setAttributes} to route association keys to their
     * associations and apply the remaining plain attributes via `super`
     * (Model's setAttributes, called directly to avoid recursing back here).
     * This makes setting attributes association-aware at construction and
     * everywhere else.
    ...
    optionsForSync(event, options={}, attributes=null) {
        ...
        options.success = (response) => {
            ...
            if( event === 'updateAttributes' ) {
                ...
                changes = this.setAttributes(response, {dirty: false});
                ...
                // setAttributes can't clobber the local edit.
