...
import {each, result, uniqueId, isEqual, deepAssign, isFunction, pick, scanPrototypesFor} from './support.js';
import {isPlainObject} from './support/object.js';
import { clone } from './support.js'
import Name from './record/name.js';
import Types from './record/types.js';
import Relation from './record/relation.js';
import * as string from './support/string.js';
...
export default class Record extends EventBus {
    ...
    static urlRoot() {
        let path = this.path;
        if (!path) {
            let routeKey = this.connection
                ? this.connection.formatRouteKey(this)
                : this.baseClass().modelName().routeKey;
            path = "/" + routeKey;
        }
        if (this.connection) return this.connection.formatPath(path);
        return path;
    }
     ...
     * @param {Record} record - The record for which the attributes are being applied
    ...
    setAttributes(attributes, {coerced=false, dirty=true}={}) {
        ...
        each(attributes, (key, value) => {
            if (!(key in this.attributes) || !isEqual(this.attributes[key], value)) {
                 ...
                 * @param {*} newValue - The new value of the attribute
                 */
                this.collections.forEach(c => c.dispatchEvent('record:changed:' + key, this, ...changes[key]));
    ...
    setAttributesAndAssociations(attributes, {dirty=true}={}) {
        ...
        return this.setAttributes(attributes, {dirty});
    }
    ...
    dumpAttributes(attributes) {
        ...
        each(attributes, (key, value) => {
            if (schema && schema[key] && schema[key].type) {
                const type = result(schema[key], 'type', attributes, this);
                const Type = Types.registry[type]
                if (!Type) {
                    throw new TypeError("Coercion of " + type + " unsupported");
                }
     ...
     */
    // TODO: some adapters don't support nested saving
    ...
    optionsForSync(event, options={}, attributes=null) {
        ...
        if (event == 'save') {
            if (options.body && isPlainObject(options.body)) {
                options.body = deepAssign(this.constructor.connection.buildRequestBody(this, this.attributesForSave()), options.body);
            } else if (!options.body) {
                options.body = this.constructor.connection.buildRequestBody(this, this.attributesForSave());
            }
        } else if (event == 'updateAttributes') {
            options.body = this.constructor.connection.buildRequestBody(this, this.dumpAttributes(attributes));
        }
        ...
        options.invalid = (request, error_callback) => {
            if (request.getResponseHeader('Content-Type').startsWith('application/json')) {
                this.errors = this.constructor.connection.parseErrors(request.responseText, request.getResponseHeader('Content-Type'));
                /**
    ...
    commit (options) {
        ...
        const wasNew = this.isNewRecord();
        const method = this.constructor.connection.method(wasNew ? 'create' : 'update');
    ...
    async reload(options = {}) {
        ...
        const method = this.constructor.connection.method('read');
        return await this.constructor.connection[method](this.url(), this.optionsForSync('reload', options));
    }
    ...
    async destroy(options = {}) {
        ...
        const method = this.constructor.connection.method('destroy');
        await this.constructor.connection[method](this.url()).then(() => {
            /**
