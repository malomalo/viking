...
//         }
    ...
    coerceAttributes(attributes, callback) {
        ...
        each(attributes, (key, value) => {
            ...
            if (schema && schema[key] && schema[key].type) {
                const coercionType = isPlainObject(schema[key].type) ? schema[key].type : Types.registry[schema[key].type]
                if (coercionType) {
                    if (schema[key].array) {
                        if (value === null || value === undefined) {
                            coercedValue = null;
                        } else {
                            coercedValue = value.map((v) => coercionType.load(v, attributes, this.attributes));
                        }
                    } else {
                        coercedValue = coercionType.load(value, attributes, this.attributes);
                    }
                } else {
                ...
                if (coercionType) {
                    if (schema[key].array) {
                        if (value === null || value === undefined) {
                            ...
                            json[key] = value.map(v => coercionType.dump(v, this.attributes));
                        }
                    } else {
                        json[key] = coercionType.dump(value, this.attributes);
                    }
                } else {
                    throw new TypeError("Coercion of " + schema[key].type + " unsupported");
                }
                ...
                if (coercionType) {
                    if (schema[key].array) {
                        if (value === null || value === undefined) {
                            ...
                            attributes[key] = value.map(v => coercionType.dump(v, this.attributes));
                        }
                    } else {
                        attributes[key] = coercionType.dump(value, this.attributes);
                    }
                } else {
                    throw new TypeError("Coercion of " + schema[key].type + " unsupported");
                }
    ...
    optionsForSave(options={}) {
        const wasNew = this.isNewRecord();
        if (wasNew) {
            this.dispatchEvent('beforeCreate', this.changedAttributes(), options)
        }
        this.dispatchEvent('beforeSave', this.changedAttributes(), options)
        
        if (options.body && isPlainObject(options.body)) {
            options.body = deepAssign({ [this.modelName.singular]: this.attributesForSave() }, options.body);
        } else if (!options.body) {
            options.body = { [this.modelName.singular]: this.attributesForSave()};
        }
        ...
        options.success = (response) => {
            ...
            each(savedChanges, (key, value) => {
                this.dispatchEvent('afterSave:'+key, value[0], value[1]);
            });
            return true;
        }
    ...
    async save(options = {}) {
        ...
        return await this.constructor.connection[method](path, this.optionsForSave(options));
