...
import {each, result, uniqueId} from './support.js';
import {ActionNotFound} from './errors.js';
...
/**
 * @typedef {Array} AroundActionCallbackWithOptions
 * @property {string|function} 0 - Callback method on the controller to call or a Function to call.
 * @property {AroundActionCallbackOptions} 1 - Options when to run the callback
 */
...
/**
 * Viking Controllers are core to determining what JavaScript is run for a URL.
 * They are made up of one or more actions that are executed on a page load.
 *
 * By default only the `ApplicationController` extends `Controller`. All
 * other controllers extend `ApplicationController`. This gives you one class to
 ...
 * ### Parameters
 * 
 * All request parameters, whether they come from the query string in the URL or
 ...
 * so that `display` doesn't get called or use an if/else statement.
 *
 * @extends EventBus
...
export default class Controller extends EventBus {
  ...
  */
   params = {};
    ...
    /**
     * Dispatches an action on the controller, running all appropriate aroundActions.
     * This method is called by the router when a route matches.
     * 
     ...
     * 
     * @param {string} action - The name of the action to process
    ...
    */
     ...
     * This renders the template as the main content of the application.
     * 
     * @param {Function} template - The template function to render
     * @param {Object} [locals={}] - Local variables to pass to the template
     * @param {Object} [options={}] - Display options to pass to the application
     * @return {Promise<Element|Element[]>} - Result of Application.display
     * @throws {DoubleRenderError} - A DoubleRenderError will be thrown if
     *                               {@link display} or {@link redirectTo} are
     *                               called multiple times in an action.
     * @example
     ...
     */
    display(template, locals={}, options={}) {
        if (this._responded) {
            throw new Errors.DoubleRenderError();
        }
        this._responded = true;
        return this.application.display(template, Object.assign({}, locals, result(this, 'helpers')), options);
    }
}