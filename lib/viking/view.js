...
export default class View extends EventBus {
    ...
    constructor(options = {}) {
        ...
        ['el', 'id', 'className', 'attributes', 'tagName', 'events', 'template'].forEach((k) => {
        ...
        this.constructor.assignProperties.forEach((k) => {
            if (options[k]) {
                this[k] = options[k];
            }
        });
    ...
    subView(SubView, ...args) {
        ...
        this.addEventListenerFor(view, 'remove', this.removeSubView);
        return view;
    }
    ...
    removeSubView(view) {
        this.subViews = this.subViews.filter((v) => { return v !== view });
