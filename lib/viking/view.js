...
import {each, result, uniqueId, scanPrototypesFor} from './support';
...
export default class View extends EventBus {
    ...
    constructor(options = {}) {
        ...
        Object.keys(this.constructor.defaults).forEach(k => {
            this[k] = options[k] ? options[k] : this.constructor.defaults[k];
        })
        ...
        if (this.el) {
            ...
            let attrs;
            // check if attributes is designated assignableProperties, thus intended for another purpose
            if (!scanPrototypesFor(this.constructor, 'assignableProperties').flat().includes('attributes')) {
                attrs = Object.assign({}, result(this, 'attributes')); //: {id?: string, class?: string}
            } else {
                attrs = {}
            }
    ...
    subView(SubView, ...args) {
        ...
        this.addEventListenerFor(view, 'removed', this.subViewRemoved);
        return view;
    }
