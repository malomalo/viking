...
export default class Association extends EventBus {
    ...
    addBang(record, options={}) {
        ...
        if (record.isPersisted()) {
            ...
            const path = this.reflection.model.connection.buildAssociationPath(this.owner, this.reflection.name);
            return this.reflection.model.connection.sendRequest(options.method, path, options).then(response => {
                if (response === false) {
    ...
    sendResourceRequest(record, options) {
        const path = this.reflection.model.connection.buildAssociationPath(this.owner, this.reflection.name, record);
        return this.reflection.model.connection.sendRequest(options.method, path, options)
    }
    ...
    reload() {
        ...
        this.loaded = false;
        let label = `${this.owner.modelName.name}[${this.owner.cid}].${this.reflection.name}[${this.cid}] Reload`
