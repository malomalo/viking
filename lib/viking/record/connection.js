...
import {each, result, last}       from '../support.js';

const requestDepths = new Set;
...
export default class Connection {
    ...
    sendRequest(method, path, {preflight = null, params = null, body = null, headers = {}, success = null, invalid = null, error = null, complete = null, progress = null, label = null} = {}) {
        let resolve, reject;
        const promise = new Promise((res, rej) => { resolve = res; reject = rej; }).finally(() => {
            console.log(
                Array.from(Array(depth-1).keys()).map((i) => requestDepths.has(i+1) ? '│      ' : '       ').join('') +
                '└END: ' + label
            )
            requestDepths.delete(depth)
        });
        ...
        const depth = ((last(Array.from(requestDepths)) || 0) + 1)
        requestDepths.add(depth)
        console.log(
            Array.from(Array(depth-1).keys()).map((i) => requestDepths.has(i+1) ? '│      ' : '       ').join('') +
            '┌START: ' + label
        )
