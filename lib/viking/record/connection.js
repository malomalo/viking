...
export default class Connection {
    ...
    sendRequest(method, path, {params = null, body = null, headers = {}, success = null, error = null, progress = null} = {}) {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.addEventListener('load', () => {
                if (request.status >= 200 && request.status <= 299) {
                    let response = request.status == 204 ? null : JSON.parse(request.response);
                    if (success) {
                    ...
                    let return_value = error(request, error_callback);
