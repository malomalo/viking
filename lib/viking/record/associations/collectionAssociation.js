...
export default class CollectionAssociation extends Association {
    ...
    setTarget(target) {
        const addedRecords = target.filter(x => {
            if (x.primaryKey()) {
                return !this.target.find(y => y.primaryKey() == x.primaryKey())
            } else {
        ...
        const removedRecords = this.target.filter(x => {
            if (x.primaryKey()) {
                return !target.find(y => y.primaryKey() == x.primaryKey())
            } else {
        ...
        if(removedRecords.length > 0) {
            ...
            removedRecords.forEach(x => x.dispatchEvent('removed', this))
        }
    ...
    mergeTarget(target) {
        ...
        target = target.map((x) => {
            let pk = x.primaryKey();
            let match = this.target.find(pk ? (y => y.primaryKey() === pk) : (y => y.cid === x.cid));
        ...
        this.target.forEach((x) => {
            let pk = x.primaryKey();
            if (!target.find(pk ? (y => y.primaryKey() === pk) : (y => y.cid === x.cid))) {
                removedRecords.push(x);
            }
        });
        ...
        commonRecords.forEach(([newRecord, oldRecord]) => {
