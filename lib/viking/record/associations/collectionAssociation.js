...
export default class CollectionAssociation extends Association {
    ...
    async load() {
        if (!this.loaded) {
            ...
            if (this.owner.primaryKey()) {
                this.mergeTarget(await this.scope().all(), false);
            } else {
    ...
    async push (record) {
        const match = await this.find(x => x.cid == record.cid)
        if (!match){
    ...
    addBang(record, options={}) {
        ...
        if (record.isPersisted()) {
            return this.sendResourceRequest(record, options).then(async () => {
                if (this.loaded) await this.add(record)
            });
        } else {
            options = record.optionsForSave(options);
            ...
            ].join("/"), options).then(async response => {
                if (response === false) {
    ...
    setAttributes(attributes, dirty) {
        ...
        this.target = attributes.map(attrs => {
            ...
            if (attrs instanceof Record) {
                ...
                record.setAttributesAndAssociations(attrs);
                record.persist();
            } else {
