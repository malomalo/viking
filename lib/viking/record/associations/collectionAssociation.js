...
export default class CollectionAssociation extends Association {
    ...
    setTarget(target) {
        const addedRecords = target.filter(x => {
            if(x.primaryKey()) return !this.target.find(y => y.primaryKey() == x.primaryKey())
        ...
        const removedRecords = this.target.filter(x => {
            if(x.primaryKey()) return !target.find(y => y.primaryKey() == x.primaryKey())
        ...
        this.target = target.map(record => {
            const existingRecord = this.target.find(x => x.primaryKey() && x.primaryKey() == record.primaryKey())
            if (existingRecord) {
                existingRecord.setAttributes(record.attributes)
