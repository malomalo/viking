...
export default class CollectionAssociation extends Association {
    ...
    setTarget(target, {dirty=true}={}) {
        ...
        this.target.splice(0, this.target.length, ...target)
        ...
        if (merged.added.length > 0) {
            ...
            merged.added.forEach(x => {
                x.collections.push(this)
                x.dispatchEvent('afterAdd', this)
            });
        }
        if (merged.removed.length > 0) {
            this.dispatchEvent('afterRemove', merged.removed);
            merged.removed.forEach(x => {
                const thisIndex = x.collections.findIndex(x => x === this)
                x.collections.splice(thisIndex, 1)
                x.dispatchEvent('afterRemove', this)
            });
        }
    }
    ...
    mergeTarget(target, dirty=true) {
        ...
        this.target.splice(0, this.target.length, ...merged.merged)
        if (this.owner.readAttribute(this.primaryKey())) {
            this.target.forEach((m) => {
                m.setAttributes({
                    [this.foreignKey()]: this.owner.readAttribute(this.primaryKey())
    ...
    setAttributes(attributes, dirty) {
        const removedRecords    = this.target.slice(0);

        let record;
        let hasPK;
        let pkOrCid;
        let matchIndex;
        let result = attributes.map((attrs, index) => {
            if (attrs instanceof Record) {
                record = attrs;
                hasPK = record.primaryKey();
                pkOrCid = hasPK ? hasPK : record.cid;
                matchIndex = removedRecords.findIndex(hasPK ? (y => y.primaryKey() === pkOrCid) : (y => y.cid === pkOrCid));
            } else {
                hasPK = attrs[this.reflection.model.primaryKey];
                if (hasPK) {
                    matchIndex = removedRecords.findIndex(y => y.primaryKey() === hasPK);
                } else {
                    matchIndex = -1;
                }
            }
            ...
            if (matchIndex >= 0) {
                let match = removedRecords.splice(matchIndex, 1)[0];
                if (record) {
                    // TODO: Should be merge models as in mergeRecords? ish
                } else {
                    match.setAttributesAndAssociations(attrs, {dirty});
                    match.persist();
                }
                return match;
            } else {
        ...
        result = result.map((index) => {
            if (index instanceof Record) {
                index.collections.push(this)
                return index;
            }
            
            let record = removedRecords.shift();
