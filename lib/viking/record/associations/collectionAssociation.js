...
import {isEqual, pick} from '../../support';
...
export default class CollectionAssociation extends Association {
    ...
    mergeRecords(oldRecords, newRecords) {
        ...
        newToOldIndexes.forEach((oldIndex, newIndex) => {
            if (oldIndex < 0) {
                ...
                if (removeUntil >= 0) {
                    deletedOldIndexes.splice(0, removeUntil-1).forEach((i) => {
    ...
    setAttributes(attributes, {coerced=false, dirty=true, inPlaceUpdate=false}={}) {
        if (!coerced) {
