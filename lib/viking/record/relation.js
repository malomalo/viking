 ...
 * |---------|-----------------------------|--------------------------|
 * | added   | record(s) added             | record(s)_added          |
 * | removed | record(s) removed           | record(s)_removed        |
 * | onload  | about to (re)load record(s) | record(s)                |
 * | loaded  | record(s) loaded            | record(s)                |
...
export default class Relation extends EventBus {
    ...
    load(options={}) {
        ...
        this.dispatchEvent('onload', this.target, options)
        ...
        this.klass.connection.get(path).then((response) => {
            ...
            if (this._calculate) {
                ...
                if(addedRecords.length > 0){
                    this.dispatchEvent('added', addedRecords, options);
                }
                
                const removedRecords = oldTarget.filter(x => !this.target.includes(x))
                if(removedRecords.length > 0) {
                    this.dispatchEvent('removed', removedRecords, options)
                }
            }
            
            this.dispatchEvent('loaded', this.target, options)
