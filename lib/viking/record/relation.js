...
export default class Relation extends EventBus {
    ...
    */
    ...
    /**
     * Build the URL for fetching this relation from the server.
     *
     * Constructs the path from the Record class's `urlRoot` and appends a
     ...
     *
     * If `options.content_type` is provided it is plucked from the options
     * and used to append a file extension to the path (e.g. `"csv"` or
     * `"text/csv"` → `".csv"`) before the query string is appended.
     *
     * @instance
     * @param {Object} [options={}] Param overrides merged over the generated
     *                              query params. `content_type` is plucked
     *                              and converted to a path extension.
     * @return {string}
     */
    toURL(options = {}) {
        const { content_type, ...paramOverrides } = options;
        ...
        if (content_type) {
            const ext = content_type.includes('/')
                ? content_type.split('/').pop()
                : content_type;
            path += `.${ext}`;
        }
    ...
    load(...eventParameters) {
        ...
        const path = this.toURL();

        if (Object.keys(params).length > 0) {
            path += `?${toQuery(params)}`;
        }
        
        const request = this.klass.connection.read(path, {label});
