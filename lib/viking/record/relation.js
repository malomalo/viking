...
import {each, result, uniqueId, merge, clone} from '../support';
        ...
        this.klass.connection.get(path).then((response) => {
            ...
            if (this._calculate) {
                ...
                this.target = response.map((r) => {
                    const record = this.klass.instantiate(r)
                    
                    this.dispatchEvent('add', record);
                    return record
                });
                
                oldTarget.filter(x => this.target.includes(x)).forEach(record => {
                    this.dispatchEvent('remove', record)
                })
            }
