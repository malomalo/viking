...
export default class Relation extends EventBus {
    ...
    */
     ...
     */
    load({callbackArgs = [], label = null}={}) {
        if (this.loaded) {
            return this.target;
        }
        if (!label) { label = this.klass.name + '['+this.cid+'].load'}
         ...
         * @param {...*} callbackArgs optional parameters from the {@link Relation#load} call
         *               that get passed to the callbacks.
         */
        this.dispatchEvent('beforeLoad', this.target, ...callbackArgs)
        ...
        const request = this.klass.connection.get(path, {label});
        ...
        this.loading.then((response) => {
             ...
             *               that get passed to the callbacks.
             */
            this.dispatchEvent('afterLoad', this.target, ...callbackArgs)
