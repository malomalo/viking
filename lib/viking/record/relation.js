...
import {each, result, uniqueId, merge} from 'viking/support';
...
export default class Relation {
    ...
    async first() {
        return this.limit(1).load().then((v) => v.length === 0 ? null : v[0]);
    }
    ...
    async length() {
        return this.load().then((v) => v.length);
    }
