 ...
 *
 * @event Viking.Relation#changed:[attribute]
 * @memberof Viking.Relation
 * @param {*} newValue The new value of the attribute
 */
...
/**
 * @class
 * @name Viking.Relation
 * @memberof Viking
 * @extends EventBus
 * @param {Viking.Record} klass A Model `class`, not an instance of the `klass`
 */
export default class Relation extends EventBus {
    ...
    */
     ...
     *
     * Use [reload]{@linkcode Viking.Relation#reload} if you wish to fetch the
     ...
     * @return {Viking.Record[]}
     *
     * @fires [beforeLoad]{@link Viking.Relation#event:beforeLoad}
     * @fires [beforeSetTarget]{@link Viking.Relation#event:beforeSetTarget}
     * @fires [afterAdd]{@link Viking.Relation#event:afterAdd}
     * @fires [afterRemove]{@link Viking.Relation#event:afterRemove}
     * @fires [afterLoad]{@link Viking.Relation#event:afterLoad}
     */
    load(...eventParameters) {
         ...
         *
         * @event Viking.Relation#beforeLoad
         * @memberof Viking.Relation
         * @param {Viking.Record[]} records The current records in memory
         * @param {...*} eventParameters optional parameters from the {@link Viking.Relation#load} call
        ...
        this.loading.then((response) => {
             ...
             *
             * @event Viking.Relation#afterLoad
             * @param {Viking.Record[]} records The current records in memory
             * @param {...*} eventParameters optional parameters from the {@link Viking.Relation#load} call
             *               that get passed to the callbacks.
             */
    ...
    setTarget(records, ...eventParameters) {
         ...
         *
         * @event Viking.Relation#beforeSetTarget
         * @param {Viking.Record[]} newRecords The records that are being set
         * @param {Viking.Record[]} currentRecords The records that are currently
         *                                         loaded
         * @param {...*} eventParameters optional parameters from the
         *               {@link Viking.Relation#setTarget} call that gets
        ...
        if (addedRecords.length > 0) {
            this.connectAddedRecords(addedRecords, ...eventParameters)
        }
        
        const removedRecords = oldTarget.filter(x => !this.target.includes(x))
        if(removedRecords.length > 0) {
            removedRecords.forEach(r => {
                const thisIndex = r.collections.findIndex(x => x === this)
                r.collections.splice(thisIndex, 1);
            })
            /**
             * Fired when records are removed to the relation.
             *
             * @event Viking.Relation#afterRemove
             * @param {Viking.Record[]} removedRecords The records that were
    ...
    add(...records) {
        ...
        this.connectAddedRecords(records)
    }

    connectAddedRecords(addedRecords, ...eventParameters) {
        addedRecords.forEach(r => r.collections.push(this))
        /**
         * Fired when records are added to the relation. Will *not* be called
         * for any records already in memory.
         *
         * @event Viking.Relation#afterAdd
         * @param {Viking.Record[]} addedRecords The records that were added
         *                          to the relation
         * @param {...*} eventParameters optional parameters from the
         *               {@link Viking.Relation#setTarget} call that gets
         *               passed to the callbacks.
    ...
    *
    * @returns {{attribute: Viking.Record[]}}
    */
    groupBy(...on) {
