...
export default class Relation extends EventBus {
    ...
    */
    ...
    /**
     * Build the path for fetching this relation from the server.
     *
     * Constructs the path from the Record connection's `path` and appends a
     * query string built from the relation's current state (where, order,
     * limit, offset, etc.). Any keys in `options` are merged over the
     * generated params, letting callers override individual query values.
     *
     * If `options.extension` is provided it is used to append a file
     * extension to the path (e.g. `"csv"`) before the query string is
     * appended.
     *
     * @instance
     * @param {Object} [options={}]
     * @param {string} [options.extension] Converted to a path extension.
     * @param {Object} [options.params={}] Param overrides merged over the
     *                                     generated query params.
     * @return {string}
     */
    toPath(options = {}) {
        const { extension = null, params: paramOverrides = {} } = options;

        let path = this.klass.connection.path(this.klass);
        let params = this.klass.connection.buildQueryParams(this);
    ...
    load(...eventParameters) {
        ...
        const label = `${this.klass.name}[${this.cid}] Load`
        ...
        const request = this.klass.connection.read(this.toPath(), {label});
     ...
     * records already in `target` when their primary key matches while updating
     * them with the response and instantiating new records otherwise.
     *
     * @instance
    ...
    *[Symbol.iterator]() {
        if (!this.loaded) {
            throw new Errors.VikingError("Relation has not been loaded. Iterate with `for await`/`Array.fromAsync`, or call load() first.");
        }
        yield* this.target;
    }
    ...
    toJSON() {
        if (!this.loaded) {
            throw new Errors.VikingError("Relation has not been loaded. Call load() before serializing.");
        }
