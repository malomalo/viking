...
import { ApiVersionUnsupported } from '../../errors.js';
...
export default class StandardAPIConnection extends AbstractConnection {
    ...
    // StandardAPI responds 400 for records that fail validation.
    invalidStatuses = [400];

    // StandardAPI responds 422 when the Api-Version header is unsupported.
    errorForResponse(request) {
        if (request.status === 422) {
            return new ApiVersionUnsupported();
        }
        return super.errorForResponse(request);
    }
    ...
    associationAttributesForSave(association) {
        switch (association.reflection.macro) {
            case 'hasMany': {
                ...
                // 
                // The server sets the foreign key from the nesting, so strip it
                const foreignKey = association.foreignKey();
                return association.target.map((record) => {
                    const attributes = this.nestedAttributesForSave(record);
          ...
          case 'hasAndBelongsToMany':
                //TODO: Only send ids if targets haven't changed
                // 
                // The foreign key lives in the join table, not on the target, so
                // there is nothing to strip from the target's attributes.
