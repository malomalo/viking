...
export default class Router extends EventBus {
    ...
    /**
     * Set the current URL query string, replacing any existing one. Remaining
     * args are passed to toParam(params, namespace, options)
     *
     * @param {Object} [params] - Query parameters to set on the current URL
     */
    setSearch(params, ...args) {
        const url = window.location.pathname + "?" + toParam(params, ...args);
