...
export default class Model extends EventBus {
    ...
    static baseModel() {
        if (
            ...
            return this.__proto__.baseModel();
    ...
    static instantiate(attributes) {
        ...
        each(association_attributes, (key, value) => {
            model[key] = model._associations[key].reflection.model.instantiate(value);
        });
...
//         }
        ...
        each(this.constructor.schema, (key, options) => {
            ...
            Object.defineProperty(this, key, {
                ...
                set: (value) => { console.log(key, value); this.setAttributes({ [key]: value }); }
            });
        });
    ...
    setAttributes(attributes) {
        ...
        each(attributes, (key, value) => {
            ...
            if (schema && schema[key] && schema[key].type) {
                if (schema[key].array) {
                    coervedValue = value.map((v) => Types.registry[schema[key].type].load(v));
                } else if (Types.registry[schema[key].type]) {
                    coervedValue = Types.registry[schema[key].type].load(value);
                } else {
            ...
            if (!isEqual(this.attributes[key], coervedValue)) {
                if (this._changes[key]) {
                    if (isEqual(this._changes[key][0], coervedValue)) {
                        delete this._changes[key];
                    } else {
                        this._changes[key][1] = coervedValue;
                    }
                } else {
                    this._changes[key] = [ this.attributes[key] || null, coervedValue ];
                }
                changes.push(key);
            }
