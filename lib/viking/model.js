...
export default class Model extends EventBus {
    ...
    // Iterable of collection-like objects (e.g. a Relation or collection
    // association) that want to be notified of this model's changes. Empty by
    // default; a consuming app can add any object with a `dispatchEvent` method
    // to track changes across multiple models.
    collections = new Set;
    ...
    /**
     * Clears the tracked changes, marking the current attributes as the clean
     * baseline. This is the in-memory analog of persisting — {@link Record#persist}
     * builds on it — without any notion of being saved to a server.
     * @returns {Model} This instance for chaining
     */
    flushChanges() {
        this._changes = {};
        return this;
    ...
    /**
     * Reverts all tracked changes, restoring attributes to the values they
     * had before they were changed.
     * @returns {Model} This instance for chaining
     */
    revertChanges() {
        const baseline = {};
        each(this._changes, (key, change) => { baseline[key] = change[0]; });
        this.setAttributes(baseline, {coerced: true});
        return this;
    }
     ...
     * then notifies each collection tracking this model.
    ...
    dispatchChange(changes) {
        ...
        /**
         * Fired on each collection tracking this model when its attributes change.
     ...
     * collection tracking this model.
    ...
    dispatchAttributeChange(key, oldValue, newValue) {
        ...
        /**
         * Fired on each collection tracking this model when a specific attribute
